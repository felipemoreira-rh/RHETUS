...
# --- 3. INICIALIZAÇÃO DE BANCO (Vagas e Candidatos) ---
...
# --- 8. GESTÃO DE VAGAS (COM EDIÇÃO) ---
    ...
    for _, row in df_v.iterrows():
        ...
        with st.container(border=True):
            ...
            # Edição
            v_key = row['id'] if 'id' in row else row['nome_vaga']
            if c3.button("📝 EDITAR", key=f"edv_{v_key}"):
                    ...
                    if st.form_submit_button("SALVAR"):
                        with engine.connect() as conn:
                            conn.execute(text("UPDATE vagas SET nome_vaga=:n, gestor=:g, status_vaga=:s, data_abertura=:da, data_fechamento=:df WHERE id=:id"),
                                         {"n": novo_n, "g": novo_g, "s": novo_s, "da": df_ab, "df": df_fc, "id": row['id']})
                            conn.commit()
                        st.session_state[f"edit_vaga_{v_key}"] = False
                        st.rerun()

# --- 9. FLUXO DE CANDIDATOS (DATAS OPCIONAIS) ---
