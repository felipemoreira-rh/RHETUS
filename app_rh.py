...
# ALLOWLIST de tabelas permitidas (previne SQL injection)
TABELAS_PERMITIDAS = {
    "vagas", "candidatos", "contratos_estagio",
    "controle_experiencia", "colaboradores_ativos",
...
def enviar_email_foto(email_candidato, nome_candidato):
    try:
        # Corrigido: e-mail e senha agora vêm de st.secrets
        ...
        <body style="font-family: sans-serif;">
            ...
            <p>👉 <strong>Gentileza preencher:</strong>
            <a href='https://docs.google.com/forms/d/e/1FAIpQLSd1o4x5jALKryUJraNB7GZB6xyJXJj5nRTs30dw_0ZFoVf9KQ/viewform?usp=sf_link'>
            Link do Formulário</a></p>
        </body>
        </html>
        """
...
# Corrigido: migrations agora têm vírgulas entre todas as strings
@st.cache_resource
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS candidatos (
                ...
                boas_vindas BOOLEAN DEFAULT FALSE,
                data_boas_vindas DATE,
                foto_curiosidades BOOLEAN DEFAULT FALSE,
                indicado_por TEXT,
                valor_bonus REAL DEFAULT 0.0
            );
        """))
        ...
        # Migrations para colunas adicionadas depois da criação original
        migrations = [
...
with st.sidebar:
    ...
    if area_sel == "RH - Recrutamento":
        ...
        # Corrigido: adicionado "📊 DASHBOARD FINANCEIRO" que existia no código mas não no menu
        menu = st.radio("NAVEGAÇÃO", ["🍔 IFOOD", "💸 OUTROS PAGAMENTOS", "📊 DASHBOARD FINANCEIRO"])
...
if menu == "📊 INDICADORES":
    ...
    if not df_v.empty:
        ...
        if not vagas_ativas.empty:
            ...
            fig_sla = px.bar(
                df_sla, x='dias_aberta', y='nome_vaga', orientation='h',
                text='dias_aberta', title="Dias em Aberto vs Limite de SLA",
                color='dias_aberta',
    ...
    with st.expander("➕ CADASTRAR NOVA VAGA"):
        with st.form("nv"):
            ...
            if st.form_submit_button("CRIAR"):
                executar_sql(
                    "INSERT INTO vagas (nome_vaga, area, status_vaga, gestor, data_abertura, empresa) VALUES (:n, :a, 'Aberta', :g, :d, :e)",
        ...
        with st.expander(f"🏢 {row['nome_vaga']} | {row.get('empresa', '---')} ({row['status_vaga']})"):
            with st.form(f"edv{row['id']}"):
                ...
                if st.form_submit_button("SALVAR ALTERAÇÕES"):
                    ...
                    executar_sql(
                        "UPDATE vagas SET nome_vaga=:nv, area=:a, status_vaga=:s, gestor=:g, data_abertura=:da, data_fechamento=:df, empresa=:e WHERE id=:id",
...
# --- 8. MÓDULO CANDIDATOS ---
    ...
    with st.expander("➕ CADASTRAR NOVO CANDIDATO"):
        with st.form("form_novo_cand_unico", clear_on_submit=True):
            ...
            v_sel = c1.selectbox("Vaga Vinculada", v_list)  # Corrigido: era vaga_sel (variável inexistente)
            ...
            if st.form_submit_button("💾 SALVAR NOVO CANDIDATO"):
                if n_novo and e_novo:
                    ...
                    """
                    executar_sql(sql_ins, {"n": n_novo, "e": e_novo, "v": v_sel, "i": ind_novo, "val": val_novo})
                    ...
                    for _, cand in df_vaga.iterrows():
                        ...
                        with st.expander(f"{icone} {cand['candidato'].upper()}"):
                            ...
                            with st.form(key=f"edit_form_final_{cand['id']}"):
                                ...
                                edit_email = col2.text_input("E-mail", value=cand.get('email', '') or '', disabled=not edit_mode)

                                v_list_edit = df_v['nome_vaga'].tolist() if not df_v.empty else ["Geral"]
                                idx_v = v_list_edit.index(cand['vaga_vinculada']) if cand['vaga_vinculada'] in v_list_edit else 0
                                edit_vaga = col1.selectbox("Vaga", v_list_edit, index=idx_v, disabled=not edit_mode)
                                ...
                                edit_ind = col1.text_input("Indicado por", value=cand.get('indicado_por', '') or '', disabled=not edit_mode)
                                edit_val = col2.number_input("Valor Bônus", value=float(cand.get('valor_bonus') or 0), disabled=not edit_mode)
                                ...
                                if b_salvar and edit_mode:
                                    ...
                                    """
                                    executar_sql(sql_up, {
                                        "n": edit_nome, "e": edit_email, "v": edit_vaga,
                                        "s": edit_status, "i": edit_ind, "val": edit_val, "id": cand['id']
                                    })
    ...
    if not df_onboarding.empty:
        for _, row in df_onboarding.iterrows():
            with st.expander(f"👤 {row['candidato']} - {row['vaga_vinculada']}"):
                ...
                if indicador and valor and valor > 0 and data_inicio:
                ...
                with st.form(key=f"onb_f_final_unlock_{row['id']}"):
                    ...
                    with col1:
                        ...
                        c_foto_email = st.checkbox(
                            "Disparar E-mail (Foto/Curiosidades)",
                    ...
                    with col2:
                        ...
                        c_foto_concluido = st.checkbox(
                            "Envio de foto e curiosidade",
...
# --- 10. MÓDULO DASHBOARD DP ---
    ...
    if not df_col.empty or not df_est.empty:
        ...
        if not df_est.empty:
            ...
            fig_prog = px.bar(
                df_prog, x='% Cumprido', y='estagiario', orientation='h',
                text='% Cumprido', color='% Cumprido',
                color_continuous_scale='RdYlGn_r', range_x=[0, 100]
            )
        ...
        with col_a1:
            ...
            if alertas_exp:
                ...
                st.success("Nenhuma avaliação de 90 dias pendente.")

        with col_a2:
            st.markdown("##### 💰 Bônus de Indicação (Pós 90 dias)")
            avisos_pagamento = []
            if not df_c.empty:
                contratados_ind = df_c[
                    (df_c['status_geral'] == 'Finalizada') &
                    (df_c['indicado_por'].notnull()) &
                    (df_c['indicado_por'] != '')
                ]
...
# --- 11. MÓDULO ESTAGIÁRIOS ---
    ...
    with col1:
        ...
        with st.form("f_est"):
            ...
            i = st.text_input("Instituição")
            t = st.selectbox("Time", ["Tecnologia", "Comercial", "Operações", "RH", "Financeiro"])
            di = st.date_input("Início")
            df_fim = st.date_input("Fim")
            if st.form_submit_button("CADASTRAR"):
                executar_sql(
                    "INSERT INTO contratos_estagio (estagiario, instituicao, data_inicio, data_fim, time_equipe) VALUES (:n, :i, :di, :df, :t)",
    ...
    with col2:
        ...
        for _, r in df_e.iterrows():
            with st.expander(f"👤 {r['estagiario']}"):
                ...
                s = ca.checkbox("Solic.", value=bool(r['solic_contrato_dp']), key=f"s{r['id']}")
                ae = cb.checkbox("ETUS", value=bool(r['assina_etus']), key=f"ae{r['id']}")
                af = cc.checkbox("Facul.", value=bool(r['assina_faculdade']), key=f"af{r['id']}")
                ej = cd.checkbox("Jurid.", value=bool(r['envio_juridico']), key=f"ej{r['id']}")
                if st.button("Salvar Status", key=f"svest{r['id']}"):
                    executar_sql(
                        "UPDATE contratos_estagio SET solic_contrato_dp=:s, assina_etus=:ae, assina_faculdade=:af, envio_juridico=:ej WHERE id=:id",
                        {"s": s, "ae": ae, "af": af, "ej": ej, "id": r['id']}
                    )
                    st.rerun()
                if st.button("🗑️ Excluir", key=f"delest{r['id']}"):
                    executar_sql("DELETE FROM contratos_estagio WHERE id=:id", {"id": r['id']})
                    st.rerun()
...
# --- 12. MÓDULO FINANCEIRO (IFOOD) ---
    ...
    with st.expander("➕ CADASTRAR NOVA NOTA IFOOD"):
        with st.form("form_ifood"):
            ...
            mni = c2.selectbox("Mês de Referência", [
...
# --- 13. MÓDULO FINANCEIRO (OUTROS PAGAMENTOS) ---
    ...
    with st.expander("➕ LANÇAR NOVO PAGAMENTO", expanded=True):
        with st.form("form_pg_geral"):
            c1, c2 = st.columns(2)
            epg = c1.selectbox("Empresa", ["Plusdin São Bernardo", "Projeto Consegui Aprender"], key="pg_e_new")
            mpg = c2.selectbox("Mês de Referência", [
                "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho",
                "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"
            ], key="pg_m_new")
...
# --- 15. MÓDULO PERÍODO DE EXPERIÊNCIA ---
    ...
    with col_cad:
        ...
        with st.form("f_exp_cad", clear_on_submit=True):
            ...
            if st.form_submit_button("CADASTRAR CONTROLE"):
                if n_exp:
                    executar_sql(
                        "INSERT INTO controle_experiencia (nome, cargo, time_equipe, data_inicio) VALUES (:n, :c, :t, :d)",
            ...
            for _, r in df_exp.iterrows():
                ...
                with st.expander(f"{status_texto} | 👤 {r['nome']} | Limite: {d90.strftime('%d/%m/%Y')}"):
                    ...
                    with c2:
                        ...
                        dt2 = st.date_input("Data da realização", value=dt2_val, key=f"dt2{r['id']}")

                    r2 = st.text_input("Avaliador responsável", value=r['av2_responsavel'] or "", key=f"r2{r['id']}", placeholder="Nome do gestor...")

                    st.markdown("---")
                    col_btn_salvar, col_btn_excluir, _ = st.columns([1, 1, 1.5])

                    with col_btn_salvar:
                        if st.button("💾 Salvar Avaliação", key=f"svexp{r['id']}", use_container_width=True):
                            executar_sql(
                                "UPDATE controle_experiencia SET av2_feito=:v2, av2_responsavel=:r2, av2_data=:dt2 WHERE id=:id",
...
# --- 16. MÓDULO COLABORADORES ---
    ...
    with st.expander("➕ CADASTRAR NOVO COLABORADOR MANUALMENTE"):
        with st.form("f_col_manual"):
            ...
            if st.form_submit_button("CADASTRAR"):
                executar_sql(
                    "INSERT INTO colaboradores_ativos (nome, tipo, data_admissao) VALUES (:n, :t, :d)",
    ...
    if not df_col.empty:
        for _, r in df_col.iterrows():
            with st.expander(f"👤 {r['nome']} [{r['tipo']}]"):
                ...
                novo_tipo = c_tipo.selectbox(
                    "Alterar Tipo", lista_modalidades,
                ...
                if col_btn1.button("💾 Salvar Alterações", key=f"svb{r['id']}"):
                    executar_sql(
                        "UPDATE colaboradores_ativos SET tipo=:t, cad_starbem=:s, incl_amil=:a, ifood_ativo=:i, equipamento_entregue=:e WHERE id=:id",
